...
class ReplayBufferedGenerator : public ReplayGeneratorBase {
 public:
  ...
  virtual ~ReplayBufferedGenerator() {
    endOfTrace_ = true;
  }

  virtual void notifyResult(const Request& req, OpResultType /*result*/) {
...
void ReplayBufferedGenerator::readReqs(){
  ...
  for(int i = 0; i<NUM_REQS; i++){
      ...
      reqs_.emplace_back(std::get<0>(reqData_[i]),std::get<1>(reqData_[i]).begin(), std::get<1>(reqData_[i]).end(), i);
  }
  for(int i = 0; i<NUM_REQS; i++){
  ...
  while(!endOfTrace_){
      ...
      doneBuffer.clear();
      doneBuffer.insert(doneBuffer.begin(), done_[*tid].begin(), done_[*tid].end());
      done_[*tid].clear();
      doneLock_[*tid].unlock();
...
const Request& ReplayBufferedGenerator::getReq(uint8_t poolId,
                                       std::mt19937_64& gen,
