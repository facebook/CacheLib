...
static int _mc_meta_save_cb(const char *tag, void *ctx, void *data) {
    struct _mc_meta_data *meta_to_save = (struct _mc_meta_data *)data;
    ...
    restart_set_kv(ctx, "slab_config", "%s", meta_to_save->slab_config);
    ...
    restart_set_kv(ctx, "mmap_oldbase", "%p", meta_to_save->mmap_base);
...
static int _mc_meta_load_cb(const char *tag, void *ctx, void *data) {
    struct _mc_meta_data *meta_candidate = (struct _mc_meta_data *)data;
    char *key;
    char *val;
    int reuse_mmap = 0;
    meta_candidate->process_started = 0;
    meta_candidate->time_delta = 0;
    meta_candidate->current_time = 0;
    int lines_seen = 0;
    ...
    while (restart_get_kv(ctx, &key, &val) == RESTART_OK) {
        ...
        switch (type) {
        case R_MMAP_OLDBASE:
            if (!safe_strtoull_hex(val, &meta_candidate->old_base)) {
        ...
        case R_SLAB_PAGE_SIZE:
            ...
            break;
        case R_SLAB_CONFIG:
            if (strcmp(val, meta_candidate->slab_config) != 0) {
                reuse_mmap = -1;
            }
            break;
        case R_USE_CAS:
        ...
        case R_PROCESS_STARTED:
            if (!safe_strtoull(val, &bigval_uint)) {
                ...
                meta_candidate->process_started = bigval_uint;
            }
            break;
        case R_CURRENT_TIME:
            if (!safe_strtoul(val, &val_uint)) {
                ...
                meta_candidate->current_time = val_uint;
            }
            break;
        case R_STOP_TIME:
            if (!safe_strtoll(val, &bigval_int)) {
                ...
                struct timeval t;
                gettimeofday(&t, NULL);
                meta_candidate->time_delta = t.tv_sec - bigval_int;
                ...
                if (meta_candidate->time_delta <= 0) {
                    reuse_mmap = -1;
                }
            }
            break;
...
//    char *memory_file = NULL;
    ...
    // moved to file scope
    meta = malloc(sizeof(struct _mc_meta_data));
    ...
    // we run memcached in in-process mode, no need to listen on connections or do clock ticks
