...
using MMWTinyLFUTest = MMTypeTest<MMWTinyLFU>;

TEST_F(MMWTinyLFUTest, AddBasic) { testAddBasic(MMWTinyLFU::Config{}); }

TEST_F(MMWTinyLFUTest, RemoveBasic) { testRemoveBasic(MMWTinyLFU::Config{}); }

TEST_F(MMWTinyLFUTest, RecordAccessBasic) {
  MMWTinyLFU::Config c;
...
TEST_F(MMWTinyLFUTest, RecordAccessWrites) {
  ...
  MMWTinyLFU::Config config1{/* lruRefreshTime */ 0,
                             /* updateOnWrite */ false,
                             /* updateOnRead */ false};
  Container c1{config1, {}};

  MMWTinyLFU::Config config2{/* lruRefreshTime */ 0,
                             /* updateOnWrite */ false,
                             /* updateOnRead */ true};
  Container c2{config2, {}};

  MMWTinyLFU::Config config3{/* lruRefreshTime */ 0,
                             /* updateOnWrite */ true,
                             /* updateOnRead */ false};
  Container c3{config3, {}};

  MMWTinyLFU::Config config4{/* lruRefreshTime */ 0,
                             /* updateOnWrite */ true,
                             /* updateOnRead */ true};
...
TEST_F(MMWTinyLFUTest, TinyLFUBasic) {
  MMWTinyLFU::Config config;
  ...
  auto checkTlfuConfig = [&](Container& container, std::string expected,
    ...
    while (it) {
      actual += folly::stringPrintf(
          "%s:%s, ", it->getKey().str().c_str(),
          (container.isTiny(*it) ? "T"
                                 : (container.isProbation(*it) ? "P" : "M")));
      ++it;
    }
  ...
  checkTlfuConfig(c, "2:P, 3:P, 4:P, 0:P, 5:P, 6:P, 7:P, 8:P, 1:T, 9:T, ",
                  "Check initial state");

  // Access an object in tiny and probation cache.
  c.recordAccess(*nodes[2], AccessMode::kRead);
  c.recordAccess(*nodes[1], AccessMode::kRead);

  // The accessed tiny items move to the head, and the accessed probation items
  // move to main
  checkTlfuConfig(c, "3:P, 4:P, 0:P, 5:P, 6:P, 7:P, 8:P, 9:T, 1:T, 2:M, ",
                  "Check after access");
  ...
  checkTlfuConfig(c, "3:P, 4:P, 0:P, 5:P, 6:P, 7:P, 8:P, 1:T, 9:T, 2:M, ",
  ...
  checkTlfuConfig(c, "1:T, 9:T, 4:P, 3:P, 0:M, 5:M, 6:M, 7:M, 8:M, 2:M, ",
  ...
  checkTlfuConfig(c, "1:P, 0:T, 2:P, 3:P, 4:P, ", "Check after adds");

  c.remove(*nodes[1]);
  checkTlfuConfig(c, "0:T, 2:P, 3:P, 4:P, ", "Check after remove from main");

  c.add(*nodes[1]);
  checkTlfuConfig(c, "1:T, 2:P, 3:P, 4:P, 0:P, ", "Check after one add");

  c.replace(*nodes[1], *nodes[6]);
  checkTlfuConfig(c, "6:T, 2:P, 3:P, 4:P, 0:P, ", "Check after replace");

  c.add(*nodes[1]);
  checkTlfuConfig(c, "2:P, 3:P, 4:P, 0:P, 6:P, 1:T, ",
                  "Check after replace and re-add");

  c.remove(*nodes[1]);
  checkTlfuConfig(c, "2:P, 3:P, 4:P, 0:P, 6:P, ",
  ...
  checkTlfuConfig(c, "9:T, 3:P, 4:P, 5:P, 2:P, 6:P, 7:P, 8:P, ",
  ...
  ////////////////// save restore ////////////////
  {
    ...
    checkTlfuConfig(newC, "3:P, 4:P, 5:P, 2:P, 6:P, 7:P, 8:P, 9:T, ",
                    "Check state after save/restore");

    // try adding at the head of tiny cache
    newC.add(*nodes[1]);
    checkTlfuConfig(newC, "3:P, 4:P, 5:P, 2:P, 6:P, 7:P, 8:P, 9:P, 1:T, ",
                    "Check state after restore and add");

    // few more operations
    newC.remove(*nodes[5]);
    checkTlfuConfig(newC, "3:P, 4:P, 2:P, 6:P, 7:P, 8:P, 9:P, 1:T, ",
...
TEST_F(MMWTinyLFUTest, SegmentStress) {
  auto doStressTest = [&](size_t windowToCacheSizeRatio,
    ...
    while (++completedOps < opsToComplete) {
      ...
      if (!inLru.empty()) {
        ...
        int actualTinySize = c.lru_.getList(MMWTinyLFU::LruType::Tiny).size();
...
TEST_F(MMWTinyLFUTest, Serialization) {
  testSerializationBasic(MMWTinyLFU::Config{});
}

TEST_F(MMWTinyLFUTest, Reconfigure) {
  Container container(MMWTinyLFU::Config{}, {});
