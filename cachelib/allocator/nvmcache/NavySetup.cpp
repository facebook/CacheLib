...
constexpr folly::StringPiece kFsSize{"dipper_fs_size"};
...
uint64_t getNvmCacheSize(const folly::dynamic& options) {
  auto fsSize = options.get_ptr(kFsSize);
  if (fsSize && fsSize->getInt() > 0) {
    return uint64_t(fsSize->getInt()) * 1024ul * 1024ul;
  }
  if (usesRaidFiles(options)) {
    auto raidPaths = options.get_ptr(kRAIDPaths);
