...
class MMS3FIFO {
 public:
  ...
  struct Config {
    ...
    // values: 1 or 2.
    uint8_t smallToMainPromoThreshold{1};
...
bool MMS3FIFO::Container<T, HookPtr>::recordAccess(T& node,
  ...
  // For Small items: lazily bump the freq counter. Promotion to Main is
...
void MMS3FIFO::Container<T, HookPtr>::lazyPromoteSmallTailLocked()
  ...
  while (smallList.size() > targetSmallSize) {
    ...
    if (getFreq(*tail) >= promoThreshold) {
      // Meets promotion threshold: move to Main head and reset counter.
