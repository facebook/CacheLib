...
TEST(RequestRangeTests, Parsing) {
  ...
  EXPECT_TRUE(hr.getRequestRange().hasValue());
  EXPECT_EQ(0, hr.getRequestRange()->first);
  EXPECT_TRUE(hr.getRequestRange()->second.hasValue());
  ...
  EXPECT_FALSE(hr.getRequestRange().hasValue());
  ...
  EXPECT_FALSE(hr.getRequestRange()->second.hasValue());
