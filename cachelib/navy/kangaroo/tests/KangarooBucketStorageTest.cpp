...
namespace tests {
namespace {
size_t getEndOffset() { return sizeof(KangarooBucketStorage); }

template <typename... T>
size_t getEndOffset(size_t n, T... args) {
  return KangarooBucketStorage::slotSize(n) + getEndOffset(args...);
}
...
TEST(KangarooBucketStorage, Allocate) {
  ...
  auto nullV = allocator->allocate(30);
  EXPECT_TRUE(nullV.done());
}

TEST(KangarooBucketStorage, Iteration) {
...
TEST(KangarooBucketStorage, RemoveFromMiddle) {
  const uint32_t capacity = 100;
  Buffer buf(capacity + sizeof(KangarooBucketStorage));
  auto* allocator1 = new (buf.data()) KangarooBucketStorage(capacity);
  {
...
TEST(KangarooBucketStorage, RemoveFromLast) {
  const uint32_t capacity = 100;
  Buffer buf(capacity + sizeof(KangarooBucketStorage));
  auto* allocator1 = new (buf.data()) KangarooBucketStorage(capacity);
  {
...
TEST(KangarooBucketStorage, RemoveUntil) {
  const uint32_t capacity = 100;
  Buffer buf(capacity + sizeof(KangarooBucketStorage));
  auto* allocator1 = new (buf.data()) KangarooBucketStorage(capacity);
  {
