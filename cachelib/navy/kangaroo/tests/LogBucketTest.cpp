...
TEST(LogBucket, DuplicateKeys) {
  ...
  // LogBucket does not replace an existing key.
...
TEST(LogBucket, Checksum) {
  ...
  const uint32_t checksum1 = LogBucket::computeChecksum(buf.view());
  bucket.setChecksum(checksum1);
  EXPECT_EQ(checksum1, bucket.getChecksum());
  EXPECT_EQ(checksum1, LogBucket::computeChecksum(buf.view()));
  ...
  EXPECT_EQ(checksum1, bucket.getChecksum());
  const uint64_t checksum2 = LogBucket::computeChecksum(buf.view());
