...
TEST(ChainedLogIndex, Chaining) {
  ...
  EXPECT_EQ(li.find(testFn(hk2.keyHash()), createTag(hk2)), PartitionOffset(2, true));
  EXPECT_EQ(li.remove(createTag(hk2), testFn(hk2.keyHash()), PartitionOffset(2, true)), Status::Ok);
  ret = li.lookup(hk0, true, &hits);
  EXPECT_EQ(ret, PartitionOffset(0, true));
  EXPECT_EQ(hits, 3);
  ret = li.lookup(hk2, false, &hits);
...
TEST(ChainedLogIndex, MergeOps) {
  ...
  li.remove(createTag(HashedKey(hk1.key())), KangarooBucketId(0), PartitionOffset(1, true));
...
TEST(ChainedLogIndex, Intermixed) {
  auto testFn = [](uint64_t hash) {
    return KangarooBucketId(hash % 3); 
  };
  ...
  EXPECT_EQ(li.lookup(makeHK("key0"), false, nullptr), PartitionOffset(0, true));
  EXPECT_EQ(li.lookup(makeHK("key1"), false, nullptr), PartitionOffset(1, true));
  EXPECT_EQ(li.lookup(makeHK("key2"), false, nullptr), PartitionOffset(2, true));
  EXPECT_EQ(li.lookup(makeHK("key3"), false, nullptr), PartitionOffset(3, true));
  EXPECT_EQ(li.lookup(makeHK("key4"), false, nullptr), PartitionOffset(4, true));
  EXPECT_EQ(li.lookup(makeHK("key5"), false, nullptr), PartitionOffset(5, true));
  EXPECT_EQ(li.lookup(makeHK("key6"), false, nullptr), PartitionOffset(6, true));
  ...
  EXPECT_EQ(li.remove(makeHK("key0"), PartitionOffset(0, true)), Status::NotFound);
  
  EXPECT_EQ(li.lookup(makeHK("key0"), false, nullptr), PartitionOffset(0, false));
  EXPECT_EQ(li.lookup(makeHK("key1"), false, nullptr), PartitionOffset(1, true));
  EXPECT_EQ(li.lookup(makeHK("key2"), false, nullptr), PartitionOffset(2, true));
  EXPECT_EQ(li.lookup(makeHK("key3"), false, nullptr), PartitionOffset(3, false));
  EXPECT_EQ(li.lookup(makeHK("key4"), false, nullptr), PartitionOffset(4, true));
  EXPECT_EQ(li.lookup(makeHK("key5"), false, &hits), PartitionOffset(5, true));
  EXPECT_EQ(li.lookup(makeHK("key6"), false, nullptr), PartitionOffset(6, false));
  EXPECT_EQ(hits, 0);
  ...
  EXPECT_EQ(li.lookup(makeHK("key3"), false, nullptr), PartitionOffset(3, false));
  EXPECT_EQ(li.lookup(makeHK("key4"), false, nullptr), PartitionOffset(4, true));
  EXPECT_EQ(li.lookup(makeHK("key5"), false, nullptr), PartitionOffset(5, false));
  EXPECT_EQ(li.lookup(makeHK("key6"), false, nullptr), PartitionOffset(6, false));
  EXPECT_EQ(li.lookup(makeHK("key7"), false, nullptr), PartitionOffset(7, true));
  EXPECT_EQ(li.lookup(makeHK("key8"), false, nullptr), PartitionOffset(8, true));
