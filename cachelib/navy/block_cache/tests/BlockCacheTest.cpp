...
TEST(BlockCache, RetryLimitSyncDeviceFailure) {
  std::vector<uint32_t> hits(4);
  auto policy = std::make_unique<NiceMock<MockPolicy>>(&hits);
  auto device = std::make_unique<NiceMock<MockDevice>>(kDeviceSize, 1024);

  // TODO: update this to always return errors when we fix T76096641
  ...
  driver->getCounters([](folly::StringPiece name, double count) {
    if (name == "navy_writer_reschedules") {
      EXPECT_EQ(3, count);
    }
  });
}

TEST(BlockCache, RetryLimitAsyncDeviceFailure) {
  std::vector<uint32_t> hits(4);
  auto policy = std::make_unique<NiceMock<MockPolicy>>(&hits);
  auto device = std::make_unique<NiceMock<MockDevice>>(kDeviceSize, 1024);
